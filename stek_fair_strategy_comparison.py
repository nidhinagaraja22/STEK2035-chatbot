...
def main():
    ...
    print(f"\n✅ Using the IDENTICAL {len(answerable_questions)} answerable questions for BOTH strategies")

    # precompute similarity ONCE per question — reused by both strategies,
    # guaranteeing identical embeddings feed into each, not just identical
    # question counts. Computed for BOTH the raw question and a generic-
    # term-stripped version, to directly test whether stripping reduces
    # the generic-overview-content contamination problem identified as
    # the leading cause of retrieval failures this session.
    print("Precomputing similarities — both RAW and STRIPPED questions...")
    questions_with_sims = []          # raw question text
    questions_with_sims_stripped = [] # generic terms removed
    for q in answerable_questions:
        ...
        if not stripped:
            stripped = q["question"]  # fallback for the rare fully-generic
                                        # case — none expected among real
                                        # answerable questions, but embedding
                                        # an empty string would error
    ...
    # ── Strategy 1: threshold-gated cascade ──────────────────────────────────
    ...
    cascade_results = evaluate_strategy("Threshold-Gated Cascade", cascade_fn, questions_with_sims)

    # ── Strategy 3: plain top-k, ZERO authority consideration at all ─────────
    # testing directly whether the pattern found earlier (softer authority
    # enforcement = better accuracy) continues all the way to NO enforcement
    def plain_topk_fn(q, sims):
        top_idx = np.argsort(-sims)[:10]
        return [doc_ids[i] for i in top_idx]

    plain_results = evaluate_strategy("Plain Top-K (no authority, UNSTRIPPED)", plain_topk_fn, questions_with_sims)
    plain_results_stripped = evaluate_strategy("Plain Top-K (no authority, STRIPPED)", plain_topk_fn, questions_with_sims_stripped)

    # ── Strategy 2: authority_source_topic (hard override) ──────────────────
    ...
    authority_results = evaluate_strategy("Authority Source Topic (hard override)", authority_fn, questions_with_sims)

    print(f"\n\n{'='*70}")
    print("HEAD-TO-HEAD, SAME 48 QUESTIONS, ALL THREE STRATEGIES")
    print(f"{'='*70}")
    ...
    print(f"\n\n{'='*70}")
    print("DOES GENERIC-TERM STRIPPING HELP? (Plain Top-K, the winning strategy)")
    print(f"{'='*70}")
    print(f"{'Metric':<15} {'Unstripped':<14} {'Stripped':<14} {'Winner'}")
    print("-" * 55)
    u_mrr, s_mrr = plain_results["mrr"], plain_results_stripped["mrr"]
    print(f"{'MRR':<15} {u_mrr:<14.4f} {s_mrr:<14.4f} {'Stripped' if s_mrr > u_mrr else 'Unstripped'}")
    for k in K_VALUES:
        for metric in ["precision", "recall", "ndcg"]:
            u = plain_results["metrics_by_k"][k][metric]
            s = plain_results_stripped["metrics_by_k"][k][metric]
            print(f"{metric}@{k:<10} {u:<14.4f} {s:<14.4f} {'Stripped' if s > u else 'Unstripped'}")

    with open("fair_strategy_comparison_results.json", "w", encoding="utf-8") as f:
        json.dump({"n_questions": len(answerable_questions),
    ...
    print(f"\n✅ Saved: fair_strategy_comparison_results.json")
