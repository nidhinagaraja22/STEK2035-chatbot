...
def clean_raw(text: str, footer_patterns: list[str],
              inline_patterns: list[str] | None = None) -> str:
    """Structural cleaning BEFORE NLP: footers, dehyphenation, whitespace.

    v2: inline_patterns are removed via re.sub anywhere in the text, because
    PDF extraction merges running footers into body text (v1 line-anchored
    filtering missed them — confirmed in doc_topic_examples.txt).
    """
    lines = text.split("\n")
    pats = [re.compile(p) for p in footer_patterns]
    lines = [ln for ln in lines if not any(p.search(ln) for p in pats)]
    text = "\n".join(lines)
    for p in (inline_patterns or []):
        text = re.sub(p, " ", text)
...
def preprocess_chunks(chunks: list[str], cfg: dict) -> list[list[str]]:
    ...
    lemma_fix = {k.lower(): v.lower()
                 for k, v in p.get("lemma_corrections", {}).items()}
...
def main():
    ...
    for f in files:
        cleaned = clean_raw(f.read_text(encoding="utf-8"),
                            ...
                            cfg["cleaning"].get("inline_patterns"))
    ...
    # ---- LDA stability check across seeds (at best k)
    stab_lines = []
    seeds = cfg["models"].get("seeds_stability", [])
    if seeds:
        topic_sets = []
        for s in seeds:
            cfg_s = {**cfg, "models": {**cfg["models"], "random_seed": s}}
            m = run_lda(docs, dictionary, corpus_bow, best_k_lda, cfg_s)
            topic_sets.append(lda_topics(m, top_n))
            log.info("Stability run seed=%d done", s)
        # greedy-match topics between seed runs by max Jaccard, report mean
        sims = []
        for a, b in combinations(range(len(topic_sets)), 2):
            for ta in topic_sets[a]:
                best = max(len(set(ta) & set(tb)) / len(set(ta) | set(tb))
                           for tb in topic_sets[b])
                sims.append(best)
        stab = float(np.mean(sims))
        stab_lines = ["", f"LDA stability (mean best-match Jaccard across "
                      f"seeds {seeds}): {stab:.3f}",
                      "  (1.0 = identical topics across seeds; >0.4 is "
                      "considered acceptably stable for small corpora)"]
        log.info("LDA cross-seed stability: %.3f", stab)
